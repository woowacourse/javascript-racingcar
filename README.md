...
- 자동자 경주 진행
  - [ ] 자동차 경주가 정상적으로 진행되는지 테스트 한다.
    > - [ ] 난수를 생성하는 함수가 0 ~ 9 사이의 정수를 반환하는지 확인하는 테스트를 한다.
    > - [ ] 전진횟수를 결정하는 함수가 '[1, 3, 3, 7]'을 입력받았을 때 '1'을 반환하는지 확인하는 테스트를 한다.
    > - [ ] 각 자동차의 이름과 화살표 갯수가 제대로 표시되는지 확인하는 테스트를 한다.
- 결과 출력
  - [ ] 자동차 경주 게임을 완료한 후 누가 우승했는지를 화면에 출력한다. 우승자가 여러명일 경우 ,를 이용하여 구분해서 출력한다.
    > - [ ] 최종 우승자가 `🏆 최종 우승자: ${쉼표로 구분된 우승자 목록} 🏆`와 일치하는지 확인하는 테스트를 한다.
  - [ ] 다시 시작버튼을 누르면 초기 화면을 출력해서 게임을 다시시작할 수 있도록 한다.
    > - [ ] '#racing-count-section', '#game-process-section', '#game-result-section'이 hidden 속성을 가지고 있는지 테스트를 한다.
    > - [ ] '#racing-count-section' 노드가 비어있는지 테스트를 한다.
    > - [ ] '#car-name-input', '#racing-count-input'의 입력창이 비어있는지 테스트를 한다.
