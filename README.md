...
- [ ] 'NAME_BLANK', 'IS_EFFECTIVE' 보다 더 명확한 이름으로 짓는다.
- [ ] 객체를 'List'는 라고 명명하지 않는다.

3. 함수명은 의도를 드러내도록

- [ ] 함수 이름은 동사 + 명사(적용할 값) 형태로 짓는다. 원래 함수의 의도를 숨기지 않는다.
- [ ] isSomething과 같이 Boolean을 반환하는 함수는 긍정문으로 작성한다.

4. 함수는 이름에 맞는 한 가지 기능만

- [ ] isValidRacingCount()에서 alert가 호출되는 사이드이펙트가 없도록 한다.

5. 사용하지 않는 반환값의 리턴은 지양

- [ ] 단지 라인수를 줄이기 위해 반환하지 않아도 될 값을 리턴문에 합쳐서 작성하지 않는다.

6. 기타

- [ ] 불필요한 new 생성자 키워드를 사용하지 않는다.
- [ ] 테스트문에 오타를 수정한다.
