...
### ✅ 2단계 Todos
- [x] try-count-btn을 클릭하면 각 자동차 이름 하단에 spinner를 띄운다.
- [x] 1초가 지날 때마다 spinner가 경주 진행 상황(화살표)로 덮어씌워진다.
- [x] 모든 자동차의 spinner가 경주 진행 상황(화살표)로 대체되면 최종 결과를 보여준다.
  - [x] 먼저 alert로 축하 메세지를 띄운다.
  - [x] alert를 없애면 최종 우승자와 함께 다시 시작하기 버튼을 화면에 띄운다.
