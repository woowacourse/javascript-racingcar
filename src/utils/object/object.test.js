...
describe('deepFreeze 함수 테스트', () => {
  ...
  describe('다중 레벨 객체 동결 테스트', () => {
    ...
    test.each(testCases)('다중 레벨 객체 내 존재하는 $targetObjectName 객체는 동결 된다.', ({ targetObject }) => {
      // then
    ...
    test('배열 내의 객체도 깊게 동결되어야 한다', () => {
      ...
      // then
      expect(
        Object.isFrozen(frozenObj) &&
          Object.isFrozen(frozenObj.array) &&
          Object.isFrozen(frozenObj.array[0]) &&
          Object.isFrozen(frozenObj.array[1]),
      ).toBeTruthy();
    });
  });
});
