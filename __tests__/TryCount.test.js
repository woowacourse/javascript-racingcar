...
import TryCount, { MAX_TRY_COUNT, MIN_TRY_COUNT } from '../src/model/TryCount.js';

describe('tryCount 횟수 입력 테스트', () => {
  // 올바른 테스트
  test.each`
    testTitle                        | tryNum  | expected
    ${'올바른 횟수가 들어온 경우 1'} | ${'1'}  | ${1}
    ${'올바른 횟수가 들어온 경우 2'} | ${'10'} | ${10}
  `(
    `${MIN_TRY_COUNT}~${MAX_TRY_COUNT} 사이의 올바른 시도 횟수가 입력되면 저장한다.`,
    ({ tryNum, expected }) => {
      const tryCount = new TryCount(tryNum);

      expect(tryCount.getTryCount()).toEqual(expected);
    },
  );
