...
describe("tryCount에 유효한 입력이 들어왔을 때", () => {
  ...
  `("$testTitle테스트는 $tryCount이 입력되면 $expected 횟수를 출력한다.", ({ tryCount, expected }) => {
...
describe("tryCount에 유효하지 않은 입력이 들어왔을 때", () => {
  test.each`
    ...
    ${"띄어쓰기만 입력하면 안된다."}         | ${" "}    | ${ERROR_MESSAGE.isNan}
  ...
  `("$testTitle테스트는 $tryCount이 입력되면 $expected 에러를 던진다.", ({ tryCount, expected }) => {
