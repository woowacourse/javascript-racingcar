...
describe('자동차 관련 유효성 검사 테스트', () => {
  describe('참여 자동차가 여러 대일 경우 쉼표로 자동차 구분', () => {
    test('쉼표로 자동차를 구분 한 경우,오류가 출력되지 않음 ', () => {
      const input = 'a,b,c';
      expect(() => CarValidator.confirm(input)).not.toThrow();
    });

    test('쉼표로 자동차를 구분하지 않은 경우 오류 출력', () => {
      const input = 'a b c';
      expect(() => CarValidator.confirm(input)).toThrow();
    });
  });
...
describe('진행횟수 관련 유효성 검사 테스트', () => {
  test('진행횟수가 1~5 사이의 값일 때', () => {
    const roundNumber = '4';
    expect(() => RoundValidator.confirmRound(roundNumber)).not.toThrow();
  });

  test('진행횟수가 1~5 사이의 값이 아닐 때', () => {
    ['a12', '6'].forEach((input) => {
      expect(() => RoundValidator.confirmRound(input)).toThrow();
    });
  });
});
