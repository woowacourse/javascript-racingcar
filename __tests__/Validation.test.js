...
const TOO_LONG_NAME = 'junjun'

describe('사용자 입력 유효성 검사', () => {
  test.each([
    [['pobi', 'jun', TOO_SHORT_NAME]],
     [[TOO_LONG_NAME, 'jun', 'cron']]
    ]
  )( '자동차 이름 길이 유효성 검사',(nameArray) => {
      expect(() => Cars.validateCarNames(nameArray)).toThrow(
  ...
  test.each([-1, -999999, 0.232, 21.5])(
    '시도 횟수 숫자 유효성 검사',
    (nameArray) => {
      expect(() => Cars.validateTryCount(nameArray)).toThrow(
