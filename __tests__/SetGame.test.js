...
describe("SetGame unit test", () => {
  ...
  test.each([["a,b,c"], ["a,b,c,d,e"], ["a"]])(
    "입력된 자동차 이름의 갯수가 1개 이상 50개 이하여야 한다. ",
    async (input) => {
  ...
  test("입력된 자동차 이름의 갯수가 1개 이상 50개가 아니면 CarNameRangeError를 반환한다.", async () => {
    InputView.readCarNames.mockResolvedValue("a,b,c");
    InputView.readCarNames.mockResolvedValueOnce("");
    InputView.readAttempt.mockResolvedValue("1");
