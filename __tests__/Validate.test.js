...
import {CAR_NAME_LIST_ERROR_MESSAGES, CAR_NAME_CONDITION, ERROR_PREFIX, ATTEMPT_ERROR_MESSAGES, RANDOM_ERROR_MESSAGES } from "../src/constants/Constants.js";
...
describe("자동차 이름 유효성 검사", () => {
  test.each(["","\n"," "])("차 이름에 빈 값이 존재하는 경우 에러 발생", (input) => {
    expect(() => validateCarName(input)).toThrow(ERROR_PREFIX+" "+CAR_NAME_LIST_ERROR_MESSAGES.NAME_LENGTH_MIN);
  });

  test.each(["수이이이이이이이이", "메타아아아아"])(
    `이름이 ${CAR_NAME_CONDITION.LENGTH_MAX}글자를 초과하는 경우 에러 발생`,
    (input) => {
      expect(() => validateCarName(input)).toThrow(ERROR_PREFIX+" "+CAR_NAME_LIST_ERROR_MESSAGES.NAME_LENGTH_MAX);
    })
...
describe("시도 횟수 유효성 검사", () => {
  test.each(["hi", ","])("문자열인 경우 에러 발생", (value) => {
    expect(() => validateAttemptCount(value)).toThrow(ERROR_PREFIX+" "+ATTEMPT_ERROR_MESSAGES.NOT_INTEGER);
  });
  ...
  test.each([0, -1])("양의 정수가 아닌 경우 에러 발생", (value) => {
    expect(() => validateAttemptCount(value)).toThrow(ERROR_PREFIX+" "+ATTEMPT_ERROR_MESSAGES.MINUS);
  });
});
...
describe("랜덤 숫자 생성 범위 유효성 검사", () => {
  test.each([[{min:"hi", max:0}, {min:0, max:"hi"}]])("문자열인 경우 에러 발생", (value) => {
    expect(() => validateRandomNumberArrange(value)).toThrow(ERROR_PREFIX+" "+RANDOM_ERROR_MESSAGES.NOT_INTEGER);
  });
  ...
  test.each([
    ...
    expect(() => validateRandomNumberArrange(value)).toThrow(ERROR_PREFIX+" "+RANDOM_ERROR_MESSAGES.MIN_GREATER_THAN_MAX);
  });
