...
describe('시도 횟수의 예외 상황 테스트', () => {
  test(`시도 횟수가 숫자가 아닌 경우 ${ErrorMessage.isNotNumber} 메시지를 포함한 에러를 throw 한다.`, () => {
    const TRY_COUNT = NaN;
  ...
  test(`시도 횟수가 정수가 아닌 경우 ${ErrorMessage.isNotInteger} 메시지를 포함한 에러를 throw 한다.`, () => {
    const TRY_COUNT = 0.5;
  ...
  test(`시도 횟수가 0 이하인 경우 ${ErrorMessage.isInvalidNumber} 메시지를 포함한 에러를 throw 한다.`, () => {
    const TRY_COUNT = 0;
  ...
  test(`시도 횟수가 100만 번을 초과한 경우 ${ErrorMessage.isBigNumber} 메시지를 포함한 에러를 throw 한다.`, () => {
