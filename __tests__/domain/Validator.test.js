...
describe('자동차 이름 입력 Validator Test', () => {
  test('isInvalidCarName Test - 이름이 6자 이상일 경우 true를 반환한다', () => {
    const CARS_NAME = 'pobi,masterwee';
    expect(isInvalidCarName(CARS_NAME)).toBe(true);
  });

  test('isInvalidCarName Test - 이름의 구분자가 쉼표(,)가 아닐 경우 true를 반환한다', () => {
    const CARS_NAME = 'pobi/jay';
    expect(isInvalidCarName(CARS_NAME)).toBe(true);
  });

  test.each(['po bi,jay', ' , ', ' '])(
    'isInvalidCarName Test - 공백 문자를 포함한 입력이 들어올 경우 true를 반환한다.',
    CARS_NAME => {
      expect(isInvalidCarName(CARS_NAME)).toBe(true);
    },
  );
  ...
  test('hasSingleCar Test - 자동차가 1개만 입력됐을 경우 true를 반환한다.', () => {
    const CAR_NAME = 'pobi';
    expect(hasSingleCar(CAR_NAME)).toBe(true);
  });
});
